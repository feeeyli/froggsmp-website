...
export function EditGroupSheet(props: EditGroupSheetProps) {
	...
	const form = useForm<z.infer<typeof schema>>({
		...
		defaultValues: {
			...
			members_ids: props.group.members.map((member) => member.id),
		},
	});

	function onSubmit(values: z.infer<typeof schema>) {
		if (
			values.slug !== props.group.slug &&
			groups.groups.some((group) => group.slug === values.slug)
		) {
			form.setError("slug", {
		...
		groups.editGroup(group, props.group.slug);
					...
					>
						...
						<SheetFooter>
							<Button
								...
								type="button"
								onClick={() => {
									groups.removeGroup(props.group.slug);
								}}
							>
								Excluir grupo <Trash size="1rem" weight="bold" />
							</Button>
							<SheetClose asChild>
								<Button
									innerClassName="gap-2"
